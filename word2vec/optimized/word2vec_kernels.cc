...
bool ScanWord(StringPiece* input, string* word) {
  ...
  //    Extract a word from input, pass it to tmp
...
class SkipgramWord2vecOp : public OpKernel {
 public:
  explicit SkipgramWord2vecOp(OpKernelConstruction* ctx)
    ...
    mutex_lock l(mu_);
    example_pos_ = corpus_size_; // corpus_size_ calculated in Init()
    label_pos_ = corpus_size_;
    label_limit_ = corpus_size_;
    sentence_index_ = kSentenceSize;
    // fill the `precalc_examples_` before the real Compute() begins
 ...
 private:
  ...
  // "tensorflow/core/platform/default/mutex.h"  based on std::mutex
  mutex mu_;
  ...
  Status Init(Env* env, const string& filename) {
    string data;
    // ReadFileToString: tensorflow/core/platform/env.h
    //   reads contents of named file into `*data`
    //
    ...
    input = data;
    // convert word of input data into word-index
    while (ScanWord(&input, &w)) {
      // tensorflow/core/lib/gtl/map_util.h
      // gtl::FindWithDefault(collection, key, value)
